...
class DatasetScannet(Dataset):
    ...
    def __init__(
        ...
        self.feature_sorted_map = {}

    def __getitem__(self, index):
        if scannet:
            return self.__getitem_scannet(index)
        return self.__getitem_hm3d(index)

    def shuffle(self, lst: list) -> list:
        indices = torch.randperm(len(lst))
        return [lst[x] for x in indices]

    def get_features_index_map(self, path):
        all_images = os.listdir(os.path.join(path, 'color'))
        sorted_indices = np.arange(len(all_images))
        sorted_images = np.sort(all_images)
        sorted_map = {img: id for (img, id) in zip(sorted_images, sorted_indices)}

        self.feature_sorted_map[path] = sorted_map


    
    def __getitem_scannet(self, idx):
        ...
        if path not in self.feature_sorted_map:
            self.get_features_index_map(path)
        
        sorted_map = self.feature_sorted_map[path]


        try:

            scene_features = torch.load(os.path.join(FEATURE_ROOT, self.sub_dir, scene, 'language_feats_256', scene+'.pt')).float()
        except:
            scene_features = torch.zeros(len(sorted_map.keys()), 256, 27, 27)
        ...
        for idx in context_index:
            ...
            feature_id = sorted_map[str(idx.numpy()) + '.jpg']
            context_features.append(scene_features[feature_id])
        ...
        context_features = F.interpolate(context_features, self.cfg.image_shape, mode='bilinear')
        ...
        for idx in target_indices:
            ...
            feature_id = sorted_map[str(idx.numpy()) + '.jpg']
            target_features.append(scene_features[feature_id])
        
        target_images = torch.cat(target_images)
        target_features = torch.stack(target_features, dim=0)
        target_features = F.interpolate(target_features, self.cfg.image_shape, mode='bilinear')
