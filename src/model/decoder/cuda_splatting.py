...
def render_cuda(
    ...
    gaussian_features: Float[Tensor, "batch gaussian 128"],
    ...
    degree = isqrt(n) - 1
    shs = rearrange(gaussian_sh_coefficients, "b g xyz n -> b g n xyz")
    ...
    for i in range(b):
        ...
        image, language_feature, radii, depth = rasterizer(
            means3D=gaussian_means[i],
            means2D=mean_gradients,
            shs=shs[i] if use_sh else None,
            colors_precomp=None if use_sh else shs[i, :, 0, :],
            language_feature_precomp=gaussian_features[i],
        ...
        all_features.append(language_feature)
