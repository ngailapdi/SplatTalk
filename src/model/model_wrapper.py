...
def feature_visualize_saving(feature):
    ...
    vis_feature = vis_feature.reshape((-1, fmap.shape[2], fmap.shape[3], 3)).permute(0,3,1,2)
    # vis_feature = vis_feature.clamp(0.0, 1.0).float().reshape((fmap.shape[2], fmap.shape[3], 3)).cpu().numpy()
    # vis_feature = (vis_feature*255).astype(np.uint8)
    # vis_feature = Image.fromarray(vis_feature)
    return vis_feature
...
class ModelWrapper(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if not isinstance(gaussians, list):
            ...
            output = DecoderOutput(None, None, None, None)
            output.color = torch.cat([x.color for x in output_list], dim=0)
            output.feature = torch.cat([x.feature for x in output_list], dim=0)
            output.updated_gaussian_feature = torch.cat([x.updated_gaussian_feature for x in output_list], dim=0)
        ...
        target_features = batch['target']['feature']
        ...
        target_indeices = batch['target']['index'].tolist()


        if batch_idx %10 == 0:
            to_print = f"train step {self.global_step}; "+\
                       f"scene = {batch['scene']}; " + \
                       f"context = {context_indices[:10]}; " +\
                       f"target = {target_indeices}; " +\
                       f"loss = {torch.mean(torch.tensor(self.loss_total)):.6f} "+\
                       f"psnr = {torch.mean(torch.tensor(psnr)):.2f} " +\
                       f"num gaussians = {encoder_results['num_gaussians']}"
            for name in self.losses_log:
                to_print = to_print + f' loss_{name} = {torch.mean(torch.tensor(self.losses_log[name])):.6f}'
            # for name in self.gradients_log:
            #     to_print = to_print + f" gradient_{name} = {torch.mean(torch.tensor(self.gradients_log[name])):.6f}"
    ...
    # features: Float[Tensor, "*batch 256"]
    ...
    def test_step(self, batch, batch_idx):
        ...
        for si, scene in enumerate(batch['scene']):
            # out_feat = self.decoder.feat_decoder(gaussians[si].features.squeeze())
            if os.path.exists('/projects/illinois/eng/cs/jrehg/users/anhthai/FreeSplatLang/output_gaussian_full_100_e75_rebuttal/%s/feat_fs/'%(scene)):
            # if os.path.exists('/projects/illinois/eng/cs/jrehg/users/anhthai/FreeSplatLang/output_gaussian_hm3d/%s/feat_fs/'%(scene)):
        ...
        with self.benchmarker.time("encoder"):
            ...
            for si, scene in enumerate(batch['scene']):
                print('scene: ', scene)
                out_feat = gaussians[si].features.squeeze()
                out_dir = '/projects/illinois/eng/cs/jrehg/users/anhthai/FreeSplatLang/output_gaussian_full_100_e75_rebuttal/%s/feat_fs/'%(scene)
                # out_dir = '/projects/illinois/eng/cs/jrehg/users/anhthai/FreeSplatLang/output_gaussian_hm3d/%s/feat_fs/'%(scene)

                os.makedirs(out_dir, exist_ok=True)
                # import pdb; pdb.set_trace()
                torch.save({'features':out_feat.detach().cpu(),
                            'points': gaussians[si].means.squeeze().cpu(),
                            'covariances': gaussians[si].covariances.squeeze().cpu(),
                             'opacitites': gaussians[si].opacities.squeeze().cpu()}, f'{out_dir}/{scene}.pt')
        ...
        with self.benchmarker.time("decoder", num_calls=v):
            if not isinstance(gaussians, list):
                ...
                for i, gs in enumerate(gaussians):
                    ...
                    now = DecoderOutput(None, None, None, None)
                    now.color = torch.cat([x.color for x in output], dim=1)
                    now.depth = torch.cat([x.depth for x in output], dim=1)
                    now.feature = torch.cat([x.feature for x in output], dim=1)
                    now.updated_gaussian_feature = torch.cat([x.updated_gaussian_feature for x in output], dim=1)
                ...
                torch.save(output.feature.squeeze(0).cpu(), path / scene/ Path(scene + '.pt'))
        ...
        context_indices = batch['context']['index'].tolist()
        target_indices = batch['target']['index'].tolist()
        print(f'Context {context_indices}')
        print(f'Target {target_indices}')
        ...
        for index, feat, feat_gt in zip(batch['target']['index'][0], output.feature[0], batch['target']['feature'][0]):

            if count < batch['target']['index'][0].shape[0] - fvs_length:
                cos = nn.CosineSimilarity(dim=0)
                out = 1- torch.mean(cos(feat, feat_gt))
                print(out)
                # feat_combined = torch.cat([feat[None,:,:,:], feat_gt[None, :, :, :]], dim=0)
                feat_combined = feat[None,:,:,:]
                # print(feat_combined.shape)
                feat_combined_pca = feature_visualize_saving(feat_combined)
                # print(feat_combined_pca.shape)
                feat_pca = feat_combined_pca[0]
                # feat_gt_pca = feat_combined_pca[1]
                save_image(feat_pca, path / scene / f"feature/{index:0>6}.png")
                # save_image(feat_gt_pca, path / scene / f"feature_gt/{index:0>6}.png")
    ...
    def test_step_orig(self, batch, batch_idx):
        print('HEREEEEEEE')
        batch: BatchedExample = self.data_shim(batch)
        # import pdb; pdb.set_trace()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if not isinstance(gaussians_probabilistic, list):
            ...
            output_probabilistic = DecoderOutput(None, None, None, None)
            # output_probabilistic.feature = torch.cat([x.feature for x in output_probabilistic_list], dim=0)
