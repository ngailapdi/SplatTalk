...
def train(cfg_dict: DictConfig):
    ...
    if cfg.output_dir is None:
        ...
        output_dir = Path(f'/projects/illinois/eng/cs/jrehg/users/anhthai/FreeSplatLang/{cfg.output_dir}')
        if not os.path.exists(output_dir):
            os.mkdir(output_dir)
        print(cyan(f"Saving outputs to {output_dir}."))
        latest_run = Path("outputs") / "latest-run-1"


    run = wandb.init(dir=str(output_dir))
